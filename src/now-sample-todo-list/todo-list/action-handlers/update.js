...
export default {
    ...
    [UPDATE_TODO]: (coeffects) => {
        const { action: { payload }, dispatch, state: { todoList }, updateState } = coeffects;
        const todo = todoList.find((todo) => todo.sys_id == payload.id);
        todo.active = "" + payload.active;
        updateState({ todoList: [...todoList] });
        dispatch(UPDATE_TODO_REQUESTED, { data: { active: payload.active }, id: payload.id });
    },

    [UPDATE_TODO_SUCCESS]: (coeffects) => {
        const { action: { payload: { result } } } = coeffects;
        console.log('UPDATE_TODO_SUCCESS....', result);
    },
