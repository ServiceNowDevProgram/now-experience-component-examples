...
export default {
    ...
    [CREATE_TODO]: (coeffects) => {
        ...
        dispatch(CREATE_TODO_REQUESTED, { data: { short_description: state.todoInputValue, assigned_to: state.user.sys_id } });
    },

    [CREATE_TODO_SUCCESS]: (coeffects) => {
        const { action: { payload: { result } }, state, updateState } = coeffects;
        updateState({ todoList: [result, ...state.todoList], todoInputValue: "", showProgress: false, alert: false });
    },
