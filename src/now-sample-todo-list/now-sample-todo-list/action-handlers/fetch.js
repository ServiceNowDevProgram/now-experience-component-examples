...
export default {
    ...
    [FETCH_TODO_SUCCESS]: (coeffects) => {
        const { action: { payload: { result: todoList} }, updateState } = coeffects;
        let alert = null;
    ...
    [USER_FETCH_SUCCESS]: ({action: {payload: {data}}, dispatch, updateState}) => {
        const {GlideDomain_Query: {user}} = data;
        updateState({user});
